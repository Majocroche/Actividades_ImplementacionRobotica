...
for k = 1:N
    ...
    dx = 1;
    dy = 2*((cos(tk)*exp(0.3*tk)) + (exp(0.3*tk)) * 0.3*sin(tk));
    
    % Velocidad lineal
    u(k) = sqrt(dx^2 + dy^2);
    
    % Velocidad angular
    w(k) = (2*((-0.91*exp(0.3*tk)) * sin(tk) + (0.6*exp(0.3*tk)) * cos(tk))) / (1 + (dy)^2);
...
axis([-1 15 -25 25 0 4]); % Ingresar limites minimos y maximos en los ejes x y z [minX maxX minY maxY minZ maxZ]
